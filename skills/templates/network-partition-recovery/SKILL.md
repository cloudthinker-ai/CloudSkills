...
enabled: true
description: |
  Use when performing network partition recovery — runbook for diagnosing and
  recovering from network partition events across distributed systems. Covers
  partition detection, impact assessment, split-brain resolution, data
  reconciliation, connectivity restoration, and post-recovery validation to
  restore full cluster consistency.
