...
enabled: true
description: |
  Use when performing architecture decision record — architecture Decision
  Record (ADR) template covering context, decision drivers, considered options,
  decision outcome, consequences, and status tracking. Based on the MADR format.
  Use for documenting significant technical decisions with full rationale.
