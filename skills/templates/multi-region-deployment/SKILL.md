...
enabled: true
description: |
  Use when performing multi region deployment — multi-region deployment template
  covering region selection, data replication strategy, traffic routing,
  failover configuration, consistency models, and operational procedures. Use
  for expanding to new regions, designing active-active architectures, or
  implementing global redundancy.
