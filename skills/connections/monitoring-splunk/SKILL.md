...
description: |
  Use when working with Splunk — splunk platform for log analytics, SPL queries,
  index management, saved searches, alerts, and dashboard analysis. Covers
  search job management, data model acceleration, KV store lookups, and
  forwarder health. Use when running SPL queries, investigating alerts,
  analyzing indexes, or managing Splunk resources via REST API.
