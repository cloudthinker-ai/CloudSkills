...
description: |
  Use when working with Sealed Secrets — bitnami Sealed Secrets management for
  Kubernetes, certificate rotation, secret encryption, and controller health.
  Covers encrypting secrets for Git storage, certificate lifecycle management,
  namespace-scoped vs cluster-wide secrets, and controller diagnostics. Use when
  encrypting Kubernetes secrets for GitOps, rotating sealing keys, auditing
  sealed secrets, or troubleshooting the sealed-secrets controller.
