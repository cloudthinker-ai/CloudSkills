...
description: |
  Google Workspace administration for user provisioning, group management, organizational unit structure, security settings review, and audit log analysis. Covers Google Admin SDK Directory API, Reports API, and security investigation. Read this skill before any Google Workspace operations — it enforces discovery-first patterns and strict read-only safety rules.
