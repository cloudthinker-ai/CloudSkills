...
description: |
  Use when working with Sagemaker — aWS SageMaker ML platform management. Covers
  training jobs, model endpoints, model registry, pipelines, feature store,
  experiments, and hyperparameter tuning. Use when managing ML training
  workflows, deploying model endpoints, investigating failed training jobs, or
  auditing SageMaker resource utilization.
connection_type: aws
preload: false
---
