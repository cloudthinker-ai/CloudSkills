...
description: |
  Use when working with Tailscale — tailscale mesh VPN management, device status
  monitoring, ACL policy analysis, DNS configuration, and exit node management.
  Covers network topology inspection, key expiry tracking, MagicDNS status, and
  subnet routing. Read this skill before any Tailscale operations — it enforces
