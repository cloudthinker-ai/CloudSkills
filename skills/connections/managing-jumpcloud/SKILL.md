...
description: |
  JumpCloud directory platform management for user and system management, SSO application configuration, device policy enforcement, group management, and audit event review. Covers LDAP, RADIUS, SSO apps, MDM policies, and system agent status. Read this skill before any JumpCloud operations — it enforces discovery-first patterns and strict read-only safety rules.
