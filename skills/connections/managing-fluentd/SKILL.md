...
description: |
  Fluentd log collector management with plugin status, buffer monitoring, routing rule analysis, match/filter configuration, and input health. Covers pipeline analysis, buffer overflow detection, retry monitoring, and configuration validation. Use when managing Fluentd plugins, analyzing buffer health, reviewing routing rules, or troubleshooting log pipelines.
